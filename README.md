...
## DEFAULT-MIME-MAP ##
This section list the default mime map used within the server. So, if you cannot find extension in the following list, or mime is not what you expected, you have to insert one or overwrite it by `--mime-map` option.

BTW, the file extension `.mjs` is not listed in the following map. I don't like it since it is very redundant to the browsers. ( I think `.mjs` is redundant since it is proposed to resolve only NodeJS's module resolution problem. ES modules ended with `.js` work fine in Browser enviroment! )
